...
export var SHELF_COORD = [[3,2], [11,2], [3,5], [11,5], [3,8], [11,8], [3,11]] # uses bottom left of shelf tiles coords
	...
	for i in range(shelf_list.size()):
		...
		for j in range(SHELF_RANGE):
			...
			if is_filled == 1:
				...
				shelf_area.position = Vector2(shelf_x * shelf_bot.cell_size.x, shelf_y * shelf_top.cell_size.y)
...
func _on_Button_pressed():
	get_tree().change_scene("res://Scenes/Randomizer Test.tscn")
