...
func evaluate_time():
	...
	if time_remaining >= 0:
		...
		reward = time_remaining / time_limit * 100.0
	
	penalties[2] = int(reward) * 5

func evaluate_performance():
	evaluate_items()
	if penalties[0] == 0 and penalties[1] == 0:
		perfect_jajan = true
	evaluate_time()
...
func update_calculation():
	calculation[0].text = str(penalties[0]) + " * (-50)"
	calculation[1].text = str(penalties[1]) + " * (-25)"
	calculation[2].text = str(penalties[2])
	calculation[5].text = str(penalties[0]*(-50) + penalties[1]*(-25) + int(total_calc[3].text) + penalties[2]) + " * " + str(reward_mult)

func update_total():
	total_calc[0].text = str(penalties[0]*-50)
	total_calc[1].text = str(penalties[1]*-25)
	total_calc[2].text = str(penalties[2])
	if perfect_jajan:
		total_calc[3].text = str(500)
	else:
		total_calc[3].text = str(0)
	total_calc[4].text = str(reward_mult)
	total_calc[5].text = str((penalties[0]*-50 + penalties[1]*-25 + int(total_calc[3].text) + penalties[2]) * reward_mult)
	total_coins_gained = (penalties[0]*-50 + penalties[1]*-25 + int(total_calc[3].text) + penalties[2]) * reward_mult
	if total_coins_gained < 0:
		total_coins_gained = 0
		total_calc[5].text = str(0)
